        ...
        <section class="l-articles row">
            <div class="column small-12">
                <div class="section-title-inner">
                    <h2 class="title-4"><?php the_field('page_title', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?></h2>
                    <select class="sortArticles-select">
                        <?php
                        if (!isset($_GET['sort']) || ($_GET['sort'] == 'date')) { ?>
                            <option selected
                                    value="<?= get_permalink(icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=date"><?php the_field('sort_last', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                            <option value="<?= get_permalink(icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <?php } else { ?>
                            <option value="<?= get_permalink(icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=date"><?php the_field('sort_last', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                            <option selected
                                    value="<?= get_permalink(icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            ...
            <div class="column small-12">
                <?php if ($_GET['sort'] == 'popular') {
                    echo do_shortcode('[ajax_load_more post_type="videos" meta_key="views" orderby="meta_value_num" order="DESC" posts_per_page="10" offset="10" pause="true" scroll="false" button_label="' . get_field('load_more_posts', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)) . '" button_loading_label="' . __('Загрузка', 'science-of-everything') . '"]');
                } else {
                    echo do_shortcode('[ajax_load_more post_type="videos" posts_per_page="10" offset="10" pause="true" scroll="false" button_label="' . get_field('load_more_posts', icl_object_id(249, 'page', true, ICL_LANGUAGE_CODE)) . '" button_loading_label="' . __('Загрузка', 'science-of-everything') . '"]');
                } ?>
            </div>
        </section>
    </div>
