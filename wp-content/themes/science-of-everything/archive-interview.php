    ...
    <section class="l-articles row">
        <div class="column small-12">
            <div class="section-title-inner">
                <h2 class="title-4"><?php the_field('page_title', icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?></h2>
                <select class="sortArticles-select">
                    <?php
                    if (!isset($_GET['sort']) || ($_GET['sort'] == 'date')) { ?>
                        <option selected
                                value="<?= get_permalink(icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=date"><?php the_field('sort_last', icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <option value="<?= get_permalink(icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                    <?php } else { ?>
                        <option value="<?= get_permalink(icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=date"><?php the_field('sort_last', icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <option selected
                                value="<?= get_permalink(icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
                    ...
                    if ($i == 0) { ?>
                        <li class="articlesList-item-hover columns column-block large-8 small-12 articlesList-medium">
                            <a class="articlesList-item-banner twoThirds" href="<?php the_permalink(); ?>">
                                <div class="articlesList-item-bg"><img class="obj-fit height-inh" src="<?php the_field('main-image'); ?>">
                                </div>
                    ...
                    <?php } else { ?>
                        <li class="articlesList-item-text articlesList-item-hover columns column-block medium-6 small-12 large-4<?php if ($i == 1) {
                            ...
                            <a class="articlesList-item-img-wrap" href="<?php the_permalink(); ?>">
                                <img class="articlesList-item-img obj-fit height-inh" style="width: 350px; height: 230px;" src="<?php the_field('main-image'); ?>"></a><a
        ...
        <div class="column small-12">
            <?php if($_GET['sort'] == 'popular') {
                echo do_shortcode('[ajax_load_more post_type="interview" meta_key="views" orderby="meta_value_num" order="DESC" posts_per_page="6" offset="7" pause="true" scroll="false" button_label="' . get_field('load_more_posts',icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)) . '" button_loading_label="' . __('Загрузка', 'science-of-everything') . '"]');
            } else {
                echo do_shortcode('[ajax_load_more post_type="interview" posts_per_page="6" offset="7" pause="true" scroll="false" button_label="' . get_field('load_more_posts', icl_object_id(657, 'page', true, ICL_LANGUAGE_CODE)) . '" button_loading_label="' . __('Загрузка', 'science-of-everything') . '"]');
            }?>
        </div>
    </section>
</div>
