<?php
/**
 * Template Name: video-collections
 */
get_header(); ?>
        ...
        <section class="l-articles row">
            <div class="column small-12">
                <div class="section-title-inner">
                    <h2 class="title-4"><?php the_field('page_title', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?></h2>
                    <select class="sortArticles-select">
                        <?php
                        if (!isset($_GET['sort']) || ($_GET['sort'] == 'date')) { ?>
                            <option selected
                                    value="<?= get_permalink(icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=date"><?php the_field('sort_last', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                            <option value="<?= get_permalink(icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <?php } else { ?>
                            <option value="<?= get_permalink(icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=date"><?php the_field('sort_last', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                            <option selected
                                    value="<?= get_permalink(icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
                    ...
                    while ($photos->have_posts()) {
                        ...
                        <?php if ($i >= 2 && $i <= 7) { ?>
                            <li class="articlesList-item-text articlesList-item-hover columns column-block medium-6 small-12 large-4 articlesList-medium-plus">
                                ...
                                --><img class="articlesList-item-img obj-fit height-inh" src="https://img.youtube.com/vi/<?php
                        ...
                        <?php } else { ?>
                            ...
                            --><a class="articlesList-item-banner center" href="<?php the_permalink(); ?>">
                                    ...
                                    --><img class="obj-fit height-inh" style="/*top: -60px;*/" src="https://img.youtube.com/vi/<?php
                                        the_field('video_id'); ?>/maxresdefault.jpg"></div><!--mqdefault-->
            ...
            <div class="column small-12">
                <?php if ($_GET['sort'] == 'popular') {
                    echo do_shortcode('[ajax_load_more post_type="video-collections" meta_key="views" orderby="meta_value_num" order="DESC" posts_per_page="10" offset="10" pause="true" scroll="false" button_label="' . get_field('load_more_posts', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)) . '" button_loading_label="' . __('Загрузка', 'science-of-everything') . '"]');
                } else {
                    echo do_shortcode('[ajax_load_more post_type="video-collections" posts_per_page="10" offset="10" pause="true" scroll="false" button_label="' . get_field('load_more_posts', icl_object_id(291, 'page', true, ICL_LANGUAGE_CODE)) . '" button_loading_label="' . __('Загрузка', 'science-of-everything') . '"]');
                } ?>
            </div>
        </section>
    </div>
