                ...
                <div class="section-title-inner">
                    ...
                    <select class="sortArticles-select">
                        ...
                        if (!isset($_GET['sort']) || ($_GET['sort'] == 'date')) { ?>
                            <option selected
                                    value="<?= get_permalink(); ?>?sort=date"><?php the_field('sort_last', icl_object_id(379, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                            <option value="<?= get_permalink(); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(379, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <?php } else { ?>
                            <option value="<?= get_permalink(); ?>?sort=date"><?php the_field('sort_last', icl_object_id(379, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                            <option selected
                                    value="<?= get_permalink(); ?>?sort=popular"><?php the_field('sort_popular', icl_object_id(379, 'page', true, ICL_LANGUAGE_CODE)); ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
