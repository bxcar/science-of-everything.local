...
$(window).on('load resize', function() {
  if ( $(window).width() <= 1440 && searchOpen ) {
    ...
    $('.header-searchForm input').on('blur', function() {
    ...
    searchOpen = false;
  } else if ( $(window).width() > 1440 && !searchOpen ) {
    $('.header-search').off('click');
    $('.header-searchForm input').off('blur');
    $('.header-searchForm, .header-searchForm input').removeAttr('style');
    $('.header-search, .header-login').removeClass('is-hidden');
...
$(window).on('load resize scroll', function() {
  if ( $(document).scrollTop() > 200 && $(window).width() >= 1120 ) {
    $('.header-menu').addClass('is-scrolled');
    $('.l-header').css('padding-top', '100px');
  } else {
    $('.header-menu').removeClass('is-scrolled');
...
if($('.header-login a').hasClass( "is-hidden" )) {
...
var bigPhotoCarousel = $('.article-gallary-slider').owlCarousel({
  ...
  onChanged: function(event) {
    $(event.target).prev('.article-gallary-labels').find('.article-gallary-counters .current').html( event.item.index + 1 );
    $(event.target).prev('.article-gallary-labels').find('.article-gallary-counters .number').html( event.item.count );
  }
  // nav: true,
...
var smallPhotoCarousel = $('.article-gallary-previews').owlCarousel({
  dots: false,
  nav: true,
  navContainerClass: 'article-gallary-previews-nav',
...
$('.article-content-text .article-gallary-photo').magnificPopup({
  type: 'image',
...
$('.booksSlider-items').owlCarousel({
  ...
  onInitialized: function(event) {
    $(event.target).prev('.booksSlider-counters').find('.current').html( event.item.index + 1 );
    $(event.target).prev('.booksSlider-counters').find('.number').html( event.item.count );
    setTimeout(function() {
      $(event.target).find('.articlesList-item-book-img').removeClass('is-readable');
      $(event.target).find('.owl-item.active:eq(0)').find('.articlesList-item-book-img').addClass('is-readable');
    }, 10)
  }
});

var booksDescriptionsSlider = $('.booksDescriptions').owlCarousel({
  items: 1,
  dots: false,
  animateOut: 'fadeOut',
  mouseDrag: false,
...
$('.booksSlider-items .owl-item').click(function(event) {
  $(event.target).closest('.owl-item').siblings().find('.articlesList-item-book-img').removeClass('is-readable');
  $(event.target).closest('.owl-item').find('.articlesList-item-book-img').addClass('is-readable');
  $(event.target).closest('.booksSlider-wrap').find('.booksSlider-counters .current').html( $(event.target).closest('.owl-item').index() + 1 );

  booksDescriptionsSlider.trigger('to.owl.carousel', [$(event.target).closest('.owl-item').index()], [500]);
})
